Feature: Service activator
  ...
  Scenario: Connection between service activator and gateway. Where gateway use only default payload transformer
    ...
    And I activate service with name "bookHandler" for "Tests\Ecotone\Messaging\Fixture\Behat\Booking\Booking" with method "book" to listen on "bookingRequest" channel
    And I activate service with name "isBookedHandler" for "Tests\Ecotone\Messaging\Fixture\Behat\Booking\Booking" with method "isBooked" to listen on "bookingConfirmation" channel and output channel "bookingConfirmationResponse"
    And I activate gateway with name "bookingGateway" for "Tests\Ecotone\Messaging\Fixture\Behat\Booking\BookingService" and "bookFlat" with request channel "bookingRequest"
    And I activate gateway with name "checkGateway" for "Tests\Ecotone\Messaging\Fixture\Behat\Booking\BookingService" and "checkIfIsBooked" with request channel "bookingConfirmation" and reply channel "bookingConfirmationResponse"
  ...
  Scenario: Connection between gateway and service activator with transformer between
    ...
    And I activate service with name "bookshopReservation" for "Tests\Ecotone\Messaging\Fixture\Behat\Shopping\Bookshop" with method "reserve" to listen on "reserveRequestTransformer" channel and output channel "reservationResponse"
    And I activate gateway with name "reserveGateway" for "Tests\Ecotone\Messaging\Fixture\Behat\Shopping\ShoppingService" and "reserve" with request channel "reserveRequest" and reply channel "reservationResponse"
    And I activate transformer with name "reservationTransformer" for "Tests\Ecotone\Messaging\Fixture\Behat\Shopping\ToReservationRequestTransformer" and "transform" with request channel "reserveRequest" and output channel "reserveRequestTransformer"
    And I run messaging system
  ...
  Scenario: Application consist of order service. It receives and order and return confirmation.
    ...
    And I activate gateway with name "orderingService" for "Tests\Ecotone\Messaging\Fixture\Behat\Ordering\OrderingService" and "processOrder" with request channel "requestChannel" and reply channel "responseChannel"
    ...
    And I activate header router with name "routing" and input Channel "routeChannel" for header "isAsync" with mapping:
      ...
      |     1 | asyncChannel   |
      |     0 | syncChannel    |
    And I activate service with name "orderProcessor" for "Tests\Ecotone\Messaging\Fixture\Behat\Ordering\OrderProcessor" with method "processOrder" to listen on "syncChannel" channel and output channel "responseChannel"
    And I run messaging system
  ...
  This will focus on asynchronous scenario.
    ...
    And I activate header router with name "routing" and input Channel "routeChannel" for header "isAsync" with mapping:
      ...
      |     1 | asyncChannel   |
      |     0 | syncChannel    |
    And I activate service with name "orderProcessor" for "Tests\Ecotone\Messaging\Fixture\Behat\Ordering\OrderProcessor" with method "processOrder" to listen on "asyncChannel" channel and output channel "responseChannel"
    And I run messaging system
    When I send order request with id 3 product name "correct" using gateway "orderingService"
  ...
  Scenario: Application consist of calculator. It receives number and perform few calculations.
    ...
    Given I active messaging for namespace "Tests\Ecotone\Messaging\Fixture\Behat\Calculating"
  ...
  Scenario: Application exception handling
    Given I active messaging for namespace "Tests\Ecotone\Messaging\Fixture\Behat\ErrorHandling"
    When I order "coffee"
  ...
  Scenario: Application handling gateways inside gateways
    Given I active messaging for namespace "Tests\Ecotone\Messaging\Fixture\Behat\GatewayInGateway"
    When I call "Tests\Ecotone\Messaging\Fixture\Behat\GatewayInGateway\CalculateGatewayExample" with 2 I should receive 76

  Scenario: Application handling interceptors for gateway
    Given I active messaging for namespace "Tests\Ecotone\Messaging\Fixture\Behat\InterceptedGateway"
    When I call "Tests\Ecotone\Messaging\Fixture\Behat\InterceptedGateway\CalculateGatewayExample" with 2 I should receive 10

  Scenario: Application handling gateways inside gateways using only messages
    Given I active messaging for namespace "Tests\Ecotone\Messaging\Fixture\Behat\GatewayInGatewayWithMessages"
    When I call with 2 I should receive 76 with message

  Scenario: Handle presend interceptor
    Given I active messaging for namespace "Tests\Ecotone\Messaging\Fixture\Behat\Presend"
  ...
  Scenario: Handle intercepted scheduled endpoint in recursion
    Given I active messaging for namespace "Tests\Ecotone\Messaging\Fixture\Behat\InterceptedScheduled"
